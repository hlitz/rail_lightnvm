...
void pblk_rail_adjust_op_cap(struct pblk *pblk)
{
	if (!pblk_rail_stride_width(pblk))
                return;

	/* Subtract parity bits from device capacity and overprovisioning */
	pblk->capacity = pblk->capacity * (pblk_rail_stride_width(pblk) - 1) /
		pblk_rail_stride_width(pblk);
	pblk->op = pblk->op * (pblk_rail_stride_width(pblk) - 1) /
		pblk_rail_stride_width(pblk);
}
...
int pblk_rail_init(struct pblk *pblk)
{
	...
	pblk->rwb.back_thres = max((int)pblk->rwb.back_thres,
				   pblk->min_write_pgs * geo->all_luns);
...
static int pblk_rail_sched_parity(struct pblk *pblk)
{
	...
	while (1) {
		...
		if (line->cur_sec >= line->emeta_ssec &&
		    line->cur_sec < (line->emeta_ssec + lm->emeta_sec[0]))
                        return 0;

		/* Skip bad blocks and meta sectors until we find a valid sec */
		if (!pblk_rail_valid_sector(pblk, line, line->cur_sec))
		       ...
		       struct bio **bio)
{
	...
	for (i = 0; i < rqd->nr_ppas; i++) {
		...
		/* We cannot split advanced bios, fall back to normal reads */
		if (bio_sectors(*bio) == 0 &&
		    (rqd->nr_ppas - i + nr_rail_ppas + pblk_rail_stride_width(pblk))
		    > NVM_MAX_VLBA)
			continue;
