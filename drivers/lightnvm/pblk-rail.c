...
{
	return (data_line->meta_distance % pblk->rail.stride_width) == 0;
}
			  ...
			  unsigned int nr_secs, unsigned int paddr)
{
	struct pblk_c_ctx *c_ctx = nvm_rq_to_pdu(rqd);
	int sec, i;
	int nr_data = pblk->rail.stride_width - 1;
	struct pblk_line *line = pblk_line_get_data(pblk);
...
static void pblk_rail_end_io_read(struct nvm_rq *rqd)
{
	...
	for (i = 0; i < nr_secs; i++) {
		...
		valid = bitmap_weight(pr_ctx->bitmap, pblk->rail.stride_width);
		bitmap_shift_right(pr_ctx->bitmap, pr_ctx->bitmap,
				   pblk->rail.stride_width,
				   NVM_MAX_VLBA * pblk->rail.stride_width);
				 ...
				 unsigned long *pvalid, int *rail_reads)
{
	...
	if (pblk_rail_lun_busy(pblk, *ppa_list) &&
		...
		bitmap_set(pvalid, valid_idx * pblk->rail.stride_width, valid);
	} else {
		rail_ppa_list[(*nr_rail_ppas)++] = *ppa_list;
		bitmap_set(pvalid, valid_idx * pblk->rail.stride_width, 1);
	}
}
		       ...
		       struct bio **bio)
{
	...
	pvalid = kcalloc(BITS_TO_LONGS(NVM_MAX_VLBA * pblk->rail.stride_width),
	...
	for (i = 0; i < rqd->nr_ppas; i++) {
		...
		if (((nr_rail_ppas + pblk->rail.stride_width) >=
