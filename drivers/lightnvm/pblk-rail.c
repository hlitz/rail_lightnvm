...
bool pblk_rail_sec_is_parity(struct pblk *pblk, struct pblk_line *line, int pos)
{
	int sec_in_stripe = pos % pblk_rail_sec_per_stripe(pblk);
	
	if (!pblk_rail_valid_sector(pblk, line, pos))
		return false;
                                 ...
                                 unsigned long *pvalid, int *rail_reads)
{
        ...
        if (pblk_rail_lun_busy(pblk, *ppa_list) &&
            pblk_rail_setup_ppas(pblk, *ppa_list,
                ...
                bitmap_set(pvalid, valid_idx * PBLK_RAIL_STRIDE_WIDTH, valid);
        } else {
                rail_ppa_list[(*nr_rail_ppas)++] = *ppa_list;
                bitmap_set(pvalid, valid_idx * PBLK_RAIL_STRIDE_WIDTH, 1);
        }
}
